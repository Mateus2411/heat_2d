# Variáveis do modelo numérico que podem ser modificadas pelo usuário:
...
nt = 10         # Número de pontos na direção t (mais frames = animação mais suave)
...
print("Fase 1: Computando a simulação...")
t_inicio = time.time()
...
print("Fase 2: Reproduzindo animação suave...")

# Fase 2: Renderização com estilo customizado
fig, ax = viz_config.configurar_figura()
extent = np.min(x), np.max(x), np.min(y), np.max(y)

im = ax.imshow(T_g[:,:,0], origin='lower', extent=extent,
               cmap=viz_config.COLORMAP,
               vmin=T0, vmax=To,
               interpolation=viz_config.INTERPOLACAO)

cbar = fig.colorbar(im)
viz_config.configurar_colorbar(cbar)

titulo = viz_config.criar_titulo(ax, f"{viz_config.TITULO_TEXTO} - t = 0.00s")
